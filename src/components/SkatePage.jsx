...
export default function SkatePage({ roomId, onLeave }) {
  ...
  const resetLogs = () => {
    if (prompt("Entrez le mot de passe 'root' :") === 'root') {
  ...
  return (
    <div style={{
      minHeight: '100vh',
      backgroundImage: 'url("https://images.unsplash.com/photo-1547448415-e9f5b97e5112?ixlib=rb-4.0.3&auto=format&fit=crop&w=1600&q=80")', // Image de minirampe
      backgroundSize: 'cover', backgroundAttachment: 'fixed', padding: '20px'
    }}>
      <div className="card" style={{ paddingTop: '80px', background: 'rgba(0,0,0,0.85)', color: '#fff' }}>
        ...
        {players.map(p => (
          <PlayerTrickEntry key={p.id} player={p} onLog={addLog} />
        ))}
...
function PlayerTrickEntry({ player, onLog }) {
