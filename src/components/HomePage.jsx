...
import { updateProfile } from 'firebase/auth'; // Pour enregistrer le pseudo de l'invité
import './HomePage.css';
...
export default function HomePage({ onUserLogin, onGameSelect, user }) {
  ...
  // États de gestion
  const [guestPseudo, setGuestPseudo] = useState(''); // Pseudo de l'invité anonyme
  ...
  const handleAnonLogin = async (e) => {
    e.preventDefault();
    if (!guestPseudo.trim()) {
      alert('Veuillez choisir un identifiant / pseudo pour continuer.');
      return;
    }
    ...
    try {
      ...
      // On sauvegarde le pseudo choisi directement dans le profil Firebase de l'utilisateur
      ...
      <div className="home-container felt-background">
        <div className="home-content">
          <div className="logo-section">
            ...
            <p className="subtitle">Gerez vos parties en temps réel</p>
          </div>
          
          <div className="login-section">
            {/* Zone Invité avec choix du pseudo */}
            <form onSubmit={handleAnonLogin} style={{ marginBottom: '20px' }}>
              <input 
                type="text"
                value={guestPseudo}
                onChange={(e) => setGuestPseudo(e.target.value)}
                placeholder="Choisissez un pseudo d'invité"
                className="join-input"
                style={{ marginBottom: '10px', textAlign: 'center' }}
              />
              <button type="submit" disabled={loading} className="btn-billard-secondary w-full text-lg py-3">
          ...
          <p className="disclaimer">Vos scores synchronisés • Jusqu'à 20 joueurs</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="home-container felt-background">
      <div className="home-content">
        <h1 className="logo-text">🎱</h1>
        <h2 className="title">Billard Scores</h2>
        <p style={{ color: '#fff', opacity: 0.8, marginBottom: '20px' }}>Bienvenue, {user.displayName || 'Invité'}</p>
