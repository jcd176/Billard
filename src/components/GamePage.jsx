...
export default function GamePage({ roomId, onLeave }) {
  ...
  return (
    <div className="card">
          ...
          {players.map((p, index) => {
            ...
            return (
              <tr key={p.id} style={{ borderBottom: '1px solid #222' }}>
                ...
                <td>{p.wins || 0} Vict / {p.losses || 0} Déf ({winRate}%)</td>
      ...
      <h3>Historique :</h3>
      <button onClick={resetLogs}>Vider</button>
      <div style={{ background: '#111', padding: '10px', borderRadius: '5px', fontSize: '14px' }}>
        {logs.map((log) => (
          <div key={log.id} style={{ marginBottom: '5px' }}>
            ...
            {log.type === 'match' ? (
              <span>👑 {log.message.split('MATCH:')[1].split('|')[0]} gagne contre 🎱 {log.message.split('|')[1]}</span>
            ) : log.type === 'leader' ? (
