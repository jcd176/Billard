...
export default function GamePage({ roomId, onLeave }) {
  ...
  useEffect(() => {
    ...
    const unsubscribePlayers = onValue(playersRef, (snapshot) => {
      ...
      // Logique "Passage en tête" restaurée
        ...
        {logs.map(log => (
          <div key={log.id} style={{ marginBottom: '5px' }}>
            ...
            {log.type === 'match' ? (
              ...
              <span style={{ color: log.type === 'add' ? '#0f0' : log.type === 'remove' ? '#f00' : log.type === 'error' ? '#EE82EE' : log.type === 'leader' ? '#FFD700' : '#FFD700' }}>{log.message}</span>
            )}
          </div>
        ))}
      </div>
