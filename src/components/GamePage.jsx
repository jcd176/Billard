...
export default function GamePage({ sport, roomId, onLeave }) {
  ...
  useEffect(() => {
    ...
    const playersRef = ref(database, `rooms/${sport}/${roomId}/players`);
    ...
    const matchesRef = ref(database, `rooms/${sport}/${roomId}/matches`);
    const unsubscribeMatches = onValue(matchesRef, (snapshot) => setMatches(snapshot.val() || {}));
  
    const logsRef = ref(database, `rooms/${sport}/${roomId}/logs`);
  ...
  const addLog = (message, type) => push(ref(database, `rooms/${sport}/${roomId}/logs`), { message, type, timestamp: Date.now() });
    ...
    if (password === 'root') {
      ...
      if (matchId) {
        ...
        if (matchOption === 'delete') {
          remove(ref(database, `rooms/${sport}/${roomId}/matches/${matchId}`));
          if (p1) update(ref(database, `rooms/${sport}/${roomId}/players/${p1.id}`), { wins: Math.max(0, (p1.wins || 0) - w1), losses: Math.max(0, (p1.losses || 0) - w2) });
          if (p2) update(ref(database, `rooms/${sport}/${roomId}/players/${p2.id}`), { wins: Math.max(0, (p2.wins || 0) - w2), losses: Math.max(0, (p2.losses || 0) - w1) });
          addLog(`Suppression partie "${matchNames}"`, 'remove');
        } else {
          set(ref(database, `rooms/${sport}/${roomId}/matches/${matchId}`), { p1: p1Name, p2: p2Name, w1: 0, w2: 0, count: 0 });
          if (p1) update(ref(database, `rooms/${sport}/${roomId}/players/${p1.id}`), { wins: Math.max(0, (p1.wins || 0) - w1), losses: Math.max(0, (p1.losses || 0) - w2) });
          if (p2) update(ref(database, `rooms/${sport}/${roomId}/players/${p2.id}`), { wins: Math.max(0, (p2.wins || 0) - w2), losses: Math.max(0, (p2.losses || 0) - w1) });
  ...
  const addPlayer = () => {
    ...
    push(ref(database, `rooms/${sport}/${roomId}/players`), { name: trimmedName, wins: 0, losses: 0 });
  ...
  const declareMatch = () => {
    ...
    update(ref(database, `rooms/${sport}/${roomId}/players/${winner}`), { wins: (wPlayer.wins || 0) + 1 });
    update(ref(database, `rooms/${sport}/${roomId}/players/${loser}`), { losses: (lPlayer.losses || 0) + 1 });
    ...
    set(ref(database, `rooms/${sport}/${roomId}/matches/${matchId}`), {
  ...
  const resetAction = (type, path) => {
    if (prompt(`Mot de passe pour vider ${type} ?`) === 'root') {
      if (type === 'classement') {
        players.forEach(p => update(ref(database, `rooms/${sport}/${roomId}/players/${p.id}`), { wins: 0, losses: 0 }));
      } else { set(ref(database, `rooms/${sport}/${roomId}/${path}`), null); }
